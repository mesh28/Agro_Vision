...
// Get crop prices for a specific crop and location
router.get('/:crop/:state/:district?', auth, async (req, res) => {
  try {
    ...
    if (district) {
      query['market.district'] = district;
    }
...
router.get('/:crop/:state/prediction', auth, async (req, res) => {
  try {
    ...
    // Generate prediction based on historical data and trends
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          highestPriceCrop: insights[0]?.name || 'N/A',
...
router.get('/:crop/:state/history', auth, async (req, res) => {
  try {
    ...
    const historicalData = priceData.historicalData.filter(
